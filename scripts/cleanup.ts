...
async function cleanup() {
    ...
    if (url && key) {
        ...
        try {
            ...
            const { error: vmErr } = await supabase.from('visual_memory').delete().neq('id', '0');
            const { error: itemsErr } = await supabase.from('order_items').delete().neq('id', '0');
            const { error: ordersErr } = await supabase.from('orders').delete().neq('id', '0');
        ...
        console.warn("⚠️ Supabase credentials bulunamadı, sadece yerel dosyalar temizlenecek.");
    }
